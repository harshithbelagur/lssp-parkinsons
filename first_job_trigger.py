...
def start_job(project_id, region, cluster, sensor_name):
    ...
    for job in jobs:
        global flag
        if (job.pyspark_job.main_python_file_uri == "gs://lssp-bucket/initial_subscriber.py") and (sensor_name in job.pyspark_job.args) and (job.status.state in do_not_start_states):
            flag = True
    ...
    if not flag:
        ...
        job = {
            ...
            'pyspark_job': {
                'main_python_file_uri': 'gs://lssp-bucket/initial_subscriber.py',
...
def hello_pubsub(cloud_event):
    ...
    region = 'us-east4'
    cluster = 'my-cluster'
    sensor_name = 'crossarms_leftwrist'
